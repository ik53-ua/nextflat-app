...
3. Edita `application.properties` y rellena tus credenciales:
...
| `application.properties` | Credenciales y config local. Está en `.gitignore`, **NUNCA se sube al repo**. |
...
| Tipo | Uso | Ejemplo |
|------|-----|---------|
| `feature` | Nueva funcionalidad | `feature/swipe-pisos` |
| `fix` | Corrección de bug en desarrollo | `fix/login-token` |
| `hotfix` | Corrección crítica en producción | `hotfix/crash-servidor` |
...
git checkout -b feature/registro-usuario          # Crear tu rama
```
...
| Comando | Descripción |
|---------|-------------|
| `./mvnw spring-boot:run` | Arranca el servidor en modo desarrollo (puerto 8080) |
| `./mvnw test` | Ejecuta los tests unitarios |
| `./mvnw package` | Genera el `.jar` de producción en `target/` |
